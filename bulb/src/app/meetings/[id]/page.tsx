...
import bgSqures from "../../../../public/bg-squares.svg";
import bgC from "../../../../public/bg-C.svg";
import bgHalfCircle from "../../../../public/bg-halfCircle.svg";
import bgLeafRight from "../../../../public/bg-leafRight.svg";
import bgLeafLeft from "../../../../public/bg-leafLeft.svg";
...
export default function MeetingPage() {
  ...
  const sendAddSection = useCallback(
    (useTitle?: boolean) => {
  ...
  return (
    <div className="flex w-screen h-screen relative">
      <div className="absolute w-screen h-screen inset-0 z-0">
        <Image className=" absolute top-1 left-1  w-[1.5%]" src={bgSqures} alt="bg-squares"></Image>

        <Image className="absolute bottom-6 left-1 w-[10%]" src={bgC} alt="bg-C"></Image>

        <Image className="absolute bottom-1 right-12" src={bgHalfCircle} alt="halfcircle"></Image>
        <Image className="absolute top-48 right-28" src={bgLeafRight} alt="leafRight"></Image>
        <Image className="absolute top-1 right-1" src={bgLeafLeft} alt="leafLeft"></Image>
        <Image className=" absolute bottom-1 right-1  w-[1.2%]" src={bgSqures} alt="bg-squares"></Image>
      </div>
      <div className="flex w-screen h-screen content-center justify-center items-center z-10">
        <div className="flex flex-row gap-10 mr-[10%] bg-white w-[calc(85%)] h-[calc(96%)] ">
                ...
                {meeting.sections.map((section: Section, index: number) => (
                  <div key={index} className="flex items-center flex-col">
                    <Tooltip
                      ...
                      isDisabled={!section.title}
                    >
                      <Button
                        isDisabled={!section.title}
                        onClick={() => scrollToTitleButtonClick(section.title)}
                        variant="light"
          ...
          <div className="flex flex-col text-primaryText gap-5 w-11/12 py-5">
            <div className="flex flex-row border-b-1 border-edge">
              <div className="flex flex-col gap-2 w-11/12">
                <Input
                  classNames={{
                    input:
                      "text-3xl font-bold text-black placeholder:text-black bg-transparent",
            ...
            <div className="flex flex-row gap-2">
              <Button
                variant="solid"
                className="bg-primaryGrey border-2 border-edge"
